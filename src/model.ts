...
class ModelField {
  ...
  public render(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillRect(
      this.parent.pos.x,
      this.parent.pos.y + FIELD_HEIGHT * (this.index + 1),
      this.parent.width,
      FIELD_HEIGHT
    );
    ctx.fillStyle = "black";
    ...
    ctx.fillText(
      this.getFieldType(),
      this.parent.pos.x + this.parent.width - 10,
      textY
    );
  }
}
function getMaxTextWidth(model: DMMF.Model, ctx: CanvasRenderingContext2D) {
  ...
  model.fields.forEach(field => {
    const width =
      padding +
      ctx.measureText(field.name).width +
      padding +
      ctx.measureText(field.type).width +
      padding;
    if (width > max) {
      max = width;
    }
...
class ModelHeader {
  ...
  public render(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillText(
      this.model.name,
      this.parent.pos.x + this.height / 2,
      this.parent.pos.y + 13
    );
  }
}
export class Model {
  ...
  constructor(ctx: CanvasRenderingContext2D, model: DMMF.Model, pos?: Vec) {
    this.pos = pos || new Vec(1000, 1000);
